  ...
  app.get<{ Params: { id: string }; Querystring: { cid?: string } }>(
    ...
    (socket: WebSocket, req) => {
      ...
      socket.on('message', (raw) => {
        try {
          ...
          if (msg.t === 'i' && typeof msg.d === 'string') {
            ...
            flushBatch();
            const needsRecovery = batchChunks.length > 0;
            resetBatch();
            detachTerminalTransport();
            releaseTerminalTransport(needsRecovery);
      ...
      const onTerminal = (data: string, cursor?: TerminalCursor) => {
        ...
        if (
          ...
          if (batchChunks.length > 0) {
            // A partial control sequence cannot cross a cursor discontinuity.
            // Drop only that unsafe suffix and force an authoritative reload.
            resetBatch();
            socket.send('{"t":"r"}');
          }
        }
      ...
      const onClearTerminal = () => {
        ...
        flushBatch();
        // A clear starts a new terminal generation. Any incomplete escape
        // suffix from the old generation is no longer meaningful.
        resetBatch();
